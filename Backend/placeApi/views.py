...
def get_places(request):
    ...
    serialized_data = PlaceSerializer(places, many=True, context={'request': request}).data
...
def create_user(request):
    ...
    if serializer.is_valid():
        logger.info("Create User - Serializer is valid")
        serializer.save()
        return Response(serializer.data, status=status.HTTP_201_CREATED)
    logger.error(f"Serializer errors: {serializer.errors}")
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
...
def user_update(request, pk):
    ...
    logger.info(f"Update User - Request method: {request.method}")
    logger.info(f"Request data: {request.data}")

    serializer = UserDetailsSerializer(user, data=request.data, partial=True)
    if serializer.is_valid():
        logger.info("Update User - Serializer is valid")
        serializer.save()
        return Response(serializer.data)
    logger.error(f"Update User - Serializer errors: {serializer.errors}")
    return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
